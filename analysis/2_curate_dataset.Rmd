...
glimpse(love_on_the_spectrum) # preview the structure of the dataset
```
```{r}
love_on_the_spectrum_files
```
...
data_dic_starter <- function(data, file_path) {
  ...
  tibble(variabe_name = names(daata), # column with existing variable names
...
data_dic_starter(data = lots_curated, # curated dataset
                 file_path = "../data/derived/love_on_the_spectrum/lots_curated_data_dictionary.csv") # ERROR HERE, GET HELP
```
